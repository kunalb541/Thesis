...
python evaluate.py --experiment-name d32_l2_hier_20241217_143022 --data test.h5
...
|--------|----------------|----------------|-------------|:----------------:|-------------|
| `distinct` | 0.1 - 1.0 | 0.8 - 1.2 | 0.01 - 0.3 | ✓ | Forced caustics for clear signals |
...
- Memory: ~2GB VRAM for batch_size=64
...
```bash
# Check data format (should be magnifications, mean ~1-2)
python -c "
import h5py
import numpy as np
...
### Missing Evaluation Plots

If u₀ dependency or temporal bias plots are missing:
...
# If missing, regenerate with simulate.py v3.0.0
