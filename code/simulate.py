...
ROMAN_SKY_MAG_AB: Final[float] = 22.0       # Sky background in F146
...
ROMAN_CADENCE_MINUTES: Final[float] = 15.0  # Roman cadence
...
# Cadence step in days for proper time grid generation
...
BINARY_MAX_MAGNIFICATION: Final[float] = 100.0  # Physical upper limit
...
# PSPL magnification cap
PSPL_MAX_MAGNIFICATION: Final[float] = 100.0

# =============================================================================
# PSPL EVENT ACCEPTANCE CRITERIA (to match Binary filtering)
...
if HAS_NUMBA:
    ...
    def flux_to_mag_numba(flux_jy: np.ndarray) -> np.ndarray:
        ...
        NOTE: This function is provided for diagnostic and post-processing
    ...
    def compute_photon_noise_numba(flux_jy: np.ndarray) -> np.ndarray:
        ...
        Implements photon noise including source flux, sky background,
    ...
    def compute_delta_t_numba(times: np.ndarray, mask: np.ndarray) -> np.ndarray:
        ...
        Notes
        -----
        The forward pass to find previous valid indices is inherently serial
        (each step depends on the last). No parallel=True as the overhead
...
class RomanWFI_F146:
    ...
    def flux_to_mag(flux_jy: np.ndarray) -> np.ndarray:
        ...
        NOTE: Provided for diagnostic/post-processing use.
...
class SimConfig:
    ...
    Time grid generation uses generate_time_grid() function
    to ensure exact 15-minute cadence using np.arange instead of np.linspace.

    Uses Roman season parameters:
    ...
    """
    TIME_MIN: float = 0.0
    TIME_MAX: float = ROMAN_SEASON_DURATION_DAYS  # 72 days
    N_POINTS: int = ROMAN_SEASON_N_POINTS  # 6912 points at 15-min cadence
...
def generate_time_grid() -> np.ndarray:
    ...
    Uses np.arange instead of np.linspace to ensure uniform
...
class BinaryPresets:
    ...
    t0 and tE ranges adjusted for 72-day season to ensure
    ...
    SHARED_U0_MIN : float
        Minimum impact parameter (Einstein radii). Shared with PSPL to prevent bias.
    SHARED_U0_MAX : float
        Maximum impact parameter (Einstein radii). Shared with PSPL to prevent bias.
    PRESETS : dict
    ...
    t_E max reduced to 18 days to ensure events complete within
    ...
    Extended t0 range to 10%-90% to include edge cases (partial events).
    All presets use SHARED_U0 range identical to PSPLParams to prevent
    the model from learning "very high magnification = binary" shortcut.

    'require_caustic' flag per preset forces caustic crossing signatures.
    'distinct' preset uses tight u0_range for better caustic intersection.
    ...
    """
    # Adjusted for 72-day season with complete events
...
class PSPLParams:
    ...
    Notes
    -----
    Uses SHARED_U0 from BinaryPresets to ensure identical
...
# =============================================================================
# CAUSTIC DETECTION AND SIZE ESTIMATION
# =============================================================================
...
def binary_magnification_vbb(
    ...
    Notes
    -----
    Return type is always normalized to np.ndarray
...
def simulate_event(params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    Binary events that fail to generate distinguishable caustic
    features return None instead of falling back to mislabeled PSPL.

    PSPL events have acceptance criteria matching Binary to
    prevent "strong event = Binary" bias.

    Binary events can optionally require caustic crossing signatures
    ...
    if etype == 'flat':
        ...
        # PSPL has acceptance criteria like Binary
        ...
        for attempt in range(PSPL_MAX_ATTEMPTS):
            ...
            # Apply acceptance criteria (similar to Binary but slightly relaxed)
        ...
        # Return None if PSPL generation failed (consistent with Binary behavior)
        ...
        for attempt in range(BINARY_MAX_ATTEMPTS):
            ...
            # For presets requiring caustic, constrain u0 based on caustic size
            if require_caustic:
                ...
                # Explicitly check for collapsed bounds
            ...
            try:
                ...
                # Additional caustic check for presets that require it
                if require_caustic:
        ...
        # Return None if binary generation failed
    ...
    # Apply photon noise in MAGNIFICATION space
    ...
    return {
        'flux': A_noisy.astype(np.float32),  # NORMALIZED magnification (baseline=1.0)
...
def worker_wrapper(args: Tuple[Dict[str, Any], int]) -> Optional[Dict[str, Any]]:
    ...
    Notes
    -----
    On failure, returns a dict with '_failed' flag and event type
    ...
    if result is None:
        # Return failure info with event type for proper attribution
...
def main() -> None:
    ...
    Time grid is generated with generate_time_grid() for exact cadence.

    Properly tracks failures by event type and uses
    ...
    # Generate time grid with exact 15-minute cadence
    ...
    # Oversample to guarantee enough events of each type
    ...
    total_gen = len(tasks)
    print("=" * 60)
    print(f"Roman Microlensing Event Simulator v{__version__}")
    print("=" * 60)
    ...
    # Track results and failures by type
    ...
    with ctx.Pool(workers) as pool:
        ...
        for res in tqdm(iterator,
            ...
            if res is None:
                ...
                # Properly attribute failure to correct type
    ...
    # Subsample to exact targets for guaranteed class balance
    ...
    # ==========================================================================
    # CHUNKED HDF5 SAVE - Memory efficient for large datasets
    # Writes in 10K-event chunks instead of allocating full array in RAM.
    # ==========================================================================
    ...
    with h5py.File(out_path, 'w') as f:
        ...
        for start_idx in tqdm(range(0, n_res, SAVE_CHUNK_SIZE), 
            ...
            # Build chunk arrays (small memory footprint)
...
if __name__ == '__main__':
    # Moved set_start_method inside main guard
